...
const RESPONSE_CODE_DESC = {
  ...
  getByCode: (statusCode: number) => { 
    if (Object.keys(RESPONSE_CODE_DESC).indexOf(statusCode.toString()) <= -1) {
      return 'Unknown Response';
    }
    let codeDesc: string | ((statusCode: number) => string) = 'Unknown Response';
    Object.entries(RESPONSE_CODE_DESC).forEach((kv) => {
      if (codeDesc !== 'Unknown Response') return;
      if (statusCode.toString() === kv[0]) {
        codeDesc = kv[1];
      }
    });
    return codeDesc;
...
const generateResponseStatus = (statusCode: number, details: string) => {
  return {
    code: statusCode,
    genericDesc: RESPONSE_CODE_DESC.getByCode(statusCode),
    details: details
  };
};

export const createResponse = (statusCode: number, details?: string, data?: object): IApiResponse => {
  return {
    status: generateResponseStatus(statusCode, details ? details : ''),
